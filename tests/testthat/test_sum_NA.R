...
test_that('sum_NA correctly returns value', {
  suppressWarnings(expect_equal(df_miss %>% summarise_all(sum_NA) %>% sum(), 30))
})

test_that('sum_NA correctly returns value', {
  suppressWarnings(expect_equal(df1 %>% summarise_all(sum_NA) %>% sum(), 0))
})


test_that('sum_NaN correctly returns value', {
  suppressWarnings(expect_equal(df_nan %>% summarise_at(dplyr::vars(a,b), sum_NaN) %>% sum(), 10))
})

test_that('sum_NaN correctly returns value', {
  suppressWarnings(expect_equal(df1 %>% summarise_at(dplyr::vars(a,b), sum_NaN) %>% sum(), 0))
})

test_that('sum_NaN fails if not a numeric', {
  suppressWarnings(expect_error(df1 %>% summarise_at(dplyr::vars(g1), sum_NaN) %>% sum()))
})

test_that('sum_blank correctly returns value', {
  suppressWarnings(expect_equal(df_blank %>% summarise_at(dplyr::vars(c), sum_blank) %>% sum(), 5))
})

test_that('sum_NaN correctly returns value', {
  suppressWarnings(expect_equal(df1 %>% summarise_at(dplyr::vars(c), sum_blank) %>% sum(), 0))
})
